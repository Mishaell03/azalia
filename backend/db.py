...
def add_sample_data():
    ...
    cursor.executemany('''
        ...
        VALUES (?, ?, ?)
    ''', [
        ('Цветущие растения', 'Растения с красивыми цветами', None),
        ('Кактусы и суккуленты', 'Засухоустойчивые растения', None),
        ('Хвойные растения', 'Сосен и елочек', None),
    ...
    cursor.executemany('''
        INSERT OR IGNORE INTO pot_plants (name, description, base_price, supplier_id, category_id, plant_type, recommended_pot_size, height_cm, care_instructions, light_requirements, watering_frequency, in_stock)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', [
        # Фикусы (category_id=4)
        ('Кудрявый фикус Барок', 'Кудрявый невысокий фикус с красивыми листьями', 1200.00, 1, 4, 'foliage', 'M', 35, 'Регулярный полив, опрыскивание', 'partial_shade', '1 раз в неделю', True),
        ('Фикус Бенджамина Вариегата', 'Фикус с белым обрамлением листочков', 1500.00, 1, 4, 'foliage', 'M', 45, 'Регулярный полив, избегать сквозняков', 'partial_shade', '1 раз в неделю', True),
        ('Фикус Natasja', 'Засухоустойчивый фикус', 1100.00, 1, 4, 'foliage', 'S', 25, 'Умеренный полив', 'partial_shade', '1 раз в 10 дней', True),
        
        # Цветущие растения (category_id=1)
        ('Фиалка Коршунова', 'Комнатное травенистое растение', 550.00, 1, 1, 'flowering', 'S', 12, 'Полив в поддон, избегать попадания воды на листья', 'partial_shade', '2-3 раза в неделю', True),
        ('Азалия Вервениана', 'Сильно ветвистое цветущее растение', 1800.00, 2, 1, 'flowering', 'M', 40, 'Регулярный полив, подкормка во время цветения', 'partial_shade', '2 раза в неделю', True),
        ('Бегония Элатиор Кармен', 'Многолетнее растение с большим количеством листьев', 850.00, 1, 1, 'flowering', 'S', 18, 'Регулярный полив, опрыскивание', 'shade', '2 раза в неделю', True),
        
        # Хвойные растения (category_id=3)
        ('Сосна горная', 'Пышная сосна с колючими веточками', 2200.00, 3, 3, 'foliage', 'L', 60, 'Редкий полив, много свежего воздуха', 'full_sun', '1 раз в 2 недели', True),
        ('Китайская ель Коника', 'Высокая ель с конусовидной формой, выведенная искусственно', 2800.00, 3, 3, 'foliage', 'L', 70, 'Умеренный полив, защита от прямого солнца', 'partial_shade', '1 раз в неделю', True),
        ('Туя', 'Медленнорастущая с компактной кроной ель с мягкими колючками', 1900.00, 3, 3, 'foliage', 'M', 50, 'Регулярный полив, опрыскивание', 'full_sun', '1 раз в неделю', True),
        
        # Кактусы и суккуленты (category_id=2)
        ('Эчеверия', 'Красивый суккулент с розеткой мясистых листьев', 650.00, 3, 2, 'succulent', 'S', 15, 'Минимальный полив, много света', 'full_sun', '1 раз в 2 недели', True),
        ('Колючая груша', 'Кактус с плоскими стеблями и колючками', 750.00, 3, 2, 'cactus', 'M', 25, 'Редкий полив, прямые солнечные лучи', 'full_sun', '1 раз в 3 недели', True),
        ('Сансевиерия', 'Неприхотливый суккулент с длинными вертикальными листьями', 500.00, 3, 2, 'succulent', 'S', 30, 'Умеренный полив, переносит тень', 'shade', '1 раз в 2 недели', True)
    ])
